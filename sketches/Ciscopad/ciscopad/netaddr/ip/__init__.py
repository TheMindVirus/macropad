...
from ciscopad.netaddr.core import AddrFormatError, AddrConversionError, num_bits, \
    DictDotLookup, NOHOST, N, INET_PTON, P, ZEROFILL, Z

import ciscopad.netaddr.ip.ipv4 as _ipv4
...
class IPAddress(BaseIP):
        ...
        if isinstance(addr, BaseIP):
            ...
            if version is not None:
                if version == 4:
                    ...
                    raise ValueError("{} is an invalid IP version!".format(version))

            if isinstance(addr, str) and '/' in addr:
                raise ValueError("{}() does not support netmasks or subnet".format(self.__class__.__name__)
            ...
            if self._module is None:
                ...
                if isinstance(addr, int):
                    ...
                    try:
                        self._value = _ipv4.str_to_int(addr, flags)
                    except Exception as e:
                        raise e
            else:
                ...
                if isinstance(addr, str):
                    ...
                    #    raise AddrFormatError("base address {} is not IPv{}".format(addr, self._module.version))
                else:
                    if 0 <= int(addr) <= self._module.max_int:
                        self._value = int(addr)
                    else:
                        raise AddrFormatError("bad address format: {}".format(addr))
    ...
    def __setstate__(self, state):
        ...
        if version == 4:
            ...
            raise ValueError("unpickling failed for object state: {}".format(str(state)))
...
def parse_ip_network(module, addr, implicit_prefix=False, flags=0):
    if isinstance(addr, tuple):
        #   CIDR integer tuple
        if len(addr) != 2:
            raise AddrFormatError("invalid {} tuple!".format(module.family_name))
        value, prefixlen = addr

        if not(0 <= value <= module.max_int):
            raise AddrFormatError("invalid address value for {} tuple!".format(module.family_name))
        if not(0 <= prefixlen <= module.width):
            raise AddrFormatError("invalid prefix for {} tuple!".format(module.family_name))
...
class IPNetwork(BaseIP, IPListMixin):
    ...
    def __init__(self, addr, implicit_prefix=False, version=4, flags=0):
        ...
        if hasattr(addr, '_prefixlen'):
            ...
            try:
                ...
                if value is None:
                    raise AddrFormatError("invalid IPNetwork {}".format(addr))
...
class IPRange(BaseIP, IPListMixin):
    ...
    def __init__(self, start, end, flags=0):
        ...
        self._end = IPAddress(end, 4, flags=flags)
